...
exports.checkEmailDuplication = async(email) => {
  try {
    ...
    const emailUser = await User.findOne({ email, status: "ACTIVE" });
    ...
    logger.error(`App - checkEmailDuplication Service error\n: ${err.message}`)
...
exports.sendCode = async(mailToSend) => {
  try {
    ...
    logger.error(`App - sendCode Service error\n: ${err.message}`)
    return errResponse(baseResponse.DB_ERROR);
  }
};
...
exports.verifyCode = async(email, validationCode) => {
  try {
    ...
    if (!mailObject) return errResponse(baseResponse.AUTH_MAIL_WRONG); // 3분 만료 됨.
    ...
    logger.error(`App - verifyCode Service error\n: ${err.message}`)
    return errResponse(baseResponse.DB_ERROR);
  }
};
